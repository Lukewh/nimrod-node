...
var constructString = function(args){
	var str = '[nimrod]['+args.timestamp+']['+args.type+']['+args.id+']['+args.value+']';
...
var confirmValue = function(value, type){
	if(type && type === 'number') {
...
var NimrodLogger = function(args){
	this.logFile = null;
	if(args && args.logFile){
...
NimrodLogger.prototype.init = function(){
	var n = this;
	// Basically a few tests
	
	// 1. Can the file be written to?
	var file = fs.openSync(this.logFile, 'a+', 0644);
	fs.closeSync(file);
};

// Can be any type
NimrodLogger.prototype.logAlert = function(args, callback){
	var n = this,
		nimrodString = null,
		stream = fs.createWriteStream(n.logFile, {
			flags: 'a+',
			encoding: 'utf8',
			mode: 0644
		});
	...
	nimrodString = constructString(args);
	...
	stream.write(nimrodString, 'utf8');
	stream.end();
};

// Logs a gauge
// Must be a number
NimrodLogger.prototype.logGauge = function(args, callback){
	var n = this;
};

// Logs a counter
// Must be a number
NimrodLogger.prototype.logCounter = function(args, callback){
	var n = this;
};

// Logs a timer
NimrodLogger.prototype.logTimer = function(args, callback){
	var n = this,
		status = 'stop';
	...
	n.timers[args.id] = status;
	
	n.logAlert(args, function(e){
		callback(e);
	});
};

// Logging default to an alert log
NimrodLogger.prototype.log = function(args, callback){
	this.logAlert(args, function(e){
		callback(e);
	});
};
